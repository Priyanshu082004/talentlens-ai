          ...
          <div key={label} className="flex-1 flex flex-col items-center relative">
            {!isLast && (
              <div className="absolute top-5 left-1/2 w-full h-0.5 bg-slate-200">
                <motion.div className="h-full bg-gradient-hero origin-left"
            ...
            <motion.div
              ...
              transition={{ duration: 1.2, repeat: active ? Infinity : 0, repeatType: 'loop' }}
              className={clsx(
                'w-10 h-10 rounded-full flex items-center justify-center border-2 relative z-10 mb-2 transition-all duration-300',
                done   && 'border-primary-500 bg-indigo-50',
                active && 'border-primary-500 bg-white shadow-glow',
            ...
            <p className={clsx('text-xs font-semibold text-center transition-colors',
              done || active ? 'text-slate-800' : 'text-slate-400')}>
              {label}
            </p>
            <p className={clsx('text-[10px] text-center mt-0.5 hidden sm:block transition-colors',
