...
export default function AIShowcase() {
  ...
  return (
    <section className="section-py relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 50% 50%, rgba(99,102,241,0.05) 0%, transparent 70%)' }} />

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
          <span className="text-xs font-semibold text-primary-400 uppercase tracking-widest mb-4 block">AI insights</span>
          <h2 className="font-display text-4xl lg:text-5xl font-bold text-white mb-4">What you get after analysis</h2>
        </motion.div>
          ...
          <div className="space-y-3">
            {PANELS.map(({ id, label, headline, sub }, i) => (
              <motion.div
                ...
                onClick={() => setActive(i)}
                className={`p-5 rounded-2xl border cursor-pointer transition-all duration-300 ${active === i ? 'bg-primary-500/10 border-primary-500/30' : 'border-white/5 hover:border-white/10 hover:bg-white/2'}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <div className={`w-2 h-2 rounded-full transition-colors ${active === i ? 'bg-primary-500' : 'bg-gray-700'}`} />
                  <span className="text-xs font-semibold text-primary-400">{label}</span>
                </div>
                <h3 className="font-display font-bold text-white text-lg mb-1">{headline}</h3>
